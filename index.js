...
const { menuss } = require('./src/menuss')
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ ESPERA UM POUCO AE MANO SOU UM ROBO 🇧🇷 ⌛',
				...
				only: {
					group: '❌ ESTE COMANDO SO PODE SER FEITO EM GRUPO! ❌',
					ownerG: '❌ Este comando só pode ser usado pelo grupo proprietário ❌',
					ownerB: '❌ Perintah ini hanya bisa di gunakan oleh owner bot! ❌',
					admin: '❌ Este comando só pode ser usado pelo proprietário do bot ❌',
					Badmin: '❌ ME COLOQUE DE ADM PARA EFETUAR ESSE COMANDO ❌'
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["21971104181@s.whatsapp.net"] // replace this with your number
      ...
      case '(mensagem recebida)':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`(https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTFNWw1_PBo-nLgvd2Yn_wefC4T2Y2saiqj1w&usqp=CAU)`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '(co do r na 😳)'})
				break
				case 'wait':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						reply(mess.wait)
					...
					break
       case 'menuss':
		client.sendMessage(from, menuss(prefix, sender), text, {quoted: mek})
      break
				default:
